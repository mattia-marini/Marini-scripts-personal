...
local topSize = 1/3
local middleSize = 1/3
local bottomSize = 1/3

function getRegion(time)
    local playheadPosition = time

    local _, numMarkers, numRegions = reaper.CountProjectMarkers(0)

    local currRegion, nextRegion = nil, nil
    local currRegionStart, currRegionend, nextRegionStart, nextRegionEnd = nil, nil, nil, nil
    for i = 0, (numMarkers + numRegions -1) do
  
        local _, isRegion, regionStart, regionEnd, regionName, rgnid = reaper.EnumProjectMarkers3(0, i)
        --reaper.ConsoleMsg(i)
        --reaper.ShowConsoleMsg(regionName == "" and "No name\n" or regionName .."\n")
        --reaper.ShowConsoleMsg(regionStart.." "..regionEnd.." "..regionName.. "\n")

        if isRegion and playheadPosition >= regionStart 
                    and playheadPosition <= regionEnd 
                    and not currRegion then
             currRegion = regionName == "" and tostring("Regione "..rgnid) or regionName
             currRegionStart, currRegionEnd = regionStart, regionEnd
        elseif  isRegion and  regionStart >= playheadPosition 
                         and playheadPosition <= regionEnd 
                         and nextRegion == nil then
            nextRegion = regionName == "" and tostring("Regione "..rgnid) or regionName
            nextRegionStart, nextRegionEnd = regionStart, regionEnd
        end
        
    end

    return currRegion or "No region", nextRegion or "No next region", currRegionStart, currRegionEnd, nextRegionStart, nextRegionEnd
end
...
local curr_region, next_region, _, _, nextRegionStart = getRegion(cursorPos)

drawTop(curr_region)

drawMiddle(next_region)

drawBottom(cursorPos, nextRegionStart)
